...
char url[] = "http://192.168.43.105/data.json"; //URL to fetch
...
void Handle_ir_code(int ir_code){
  if(ir_code==-133697792){
      on0=inv(on0);
      if(on0)
        req("http://192.168.43.105/api/flip/on0/true");
      else
        req("http://192.168.43.105/api/flip/on0/false");
    }
    if(ir_code==-116986112){
      on1=inv(on1);
      if(on1)
        req("http://192.168.43.105/api/flip/on1/true");
      else
        req("http://192.168.43.105/api/flip/on1/false");
    }
    if(ir_code==-100274432){
      on2=inv(on2);
      if(on2)
        req("http://192.168.43.105/api/flip/on2/true");
      else
        req("http://192.168.43.105/api/flip/on2/false");
    }
    if(ir_code==-83562752){
      on3=inv(on3);
      if(on3)
        req("http://192.168.43.105/api/flip/on3/true");
      else
        req("http://192.168.43.105/api/flip/on3/false");
    }
}
